...
const httpCultivos = {
    getCultivos: async (req, res) => {
        try {
            const cultivos = await Cultivo.find().populate({ path: "id_parcela", select: "numero id_fincas", populate: { path: "id_fincas", select: "nombre rut"}});
            res.json({ cultivos });
        } catch (error) {
    ...
    getCultivosTipo: async (req, res) => {
        try {
            ...
            const cultivos = await Cultivo.find({ tipo }).populate({ path: "id_parcela", select: "numero id_fincas", populate: { path: "id_fincas", select: "nombre rut"}});
            res.json({ cultivos });
        } catch (error) {
            res.json({ err: error.message });
        }
    ...
    getCultivosParcela: async (req, res) => {
        try {
            ...
            const cultivos = await Cultivo.find({ id_parcela }).populate({ path: "id_parcela", select: "numero id_fincas", populate: { path: "id_fincas", select: "nombre rut"}});
            res.json({ cultivos });
        } catch (error) {
            res.json({ err: error.message });
        }
