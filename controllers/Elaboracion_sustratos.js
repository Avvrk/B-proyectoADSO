...
const httpSustratos = {
	getSustratos: async (req, res) => {
		try {
			const sustratos = await Sustrato.find()
				.populate("id_proceso", "tipo")
				.populate("empleado_idOperario", "nombre correo")
				.populate("empleado_idResponsable", "nombre correo");
			res.json({ sustratos });
		} catch (error) {
	...
	putSustratos: async (req, res) => {
		try {
			const { id } = req.params;
			const { ...info } = req.body;
			const sustratos = await Sustrato.findByIdAndUpdate(id, info, {
				new: true,
			});
			res.json({ sustratos });
		} catch (error) {
