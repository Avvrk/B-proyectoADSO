...
const httpRiegos = {
	...
	getCantidadAguaPorCultivo: async (req, res) => {
		try {
			...
			const riegos = await Riego.find({ cultivo_id })
				.populate("cultivo_id", "nombre")
				.populate("empleado_id", "nombre");
			const cantidadAguaTotal = riegos.reduce(
				(total, riego) => total + riego.cantidad_agua,
				0
			);
	...
	putRiego: async (req, res) => {
		try {
			const { id } = req.params;
			const { ...info } = req.body;
			const riego = await Riego.findByIdAndUpdate(id, info, {
				new: true,
			});
			res.json({ riego });
		} catch (error) {
	...
	putRiegoActivar: async (req, res) => {
		try {
			const { id } = req.params;
			const riego = await Riego.findByIdAndUpdate(
				id,
				{ estado: 1 },
				{ new: true }
			);
