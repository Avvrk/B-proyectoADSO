...
const httpEmpleados = {
    ...
    getEmpleadosDescripcion: async (req, res) => {
        try {
            const { id } = req.params;
            const empleados = await Empleado.find({ _id: id }, { descripcion: 1 });
            res.json({ empleados });
        } catch (error) {
            res.json({ error });
        }
    ...
    putEmpleadosActivar: async (req, res) => {
        try {
            const { id } = req.params;
            const empleados = await Empleado.findByIdAndUpdate(id, { estado: 1 }, { new: true });
            res.json({ empleados });
        } catch (error) {
            res.json({ error });
        }
    ...
    putEmpleadosInactivar: async (req, res) => {
        try {
            const { id } = req.params;
            const empleados = await Empleado.findByIdAndUpdate(id, { estado: 0 }, { new: true });
            res.json({ empleados });
        } catch (error) {
            res.json({ error });
        }
