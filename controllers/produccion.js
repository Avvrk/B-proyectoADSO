...
const httpProducciones = {
	getProducciones: async (req, res) => {
		try {
			const producciones = await Produccion.find().populate(
				"cultivo_id",
				"nombre"
			);
	...
	getProduccionId: async (req, res) => {
		try {
			const { id } = req.params;
			const produccion = await Produccion.findById(id).populate(
				"cultivo_id",
				"nombre"
			);
			res.json({ produccion });
	...
	getProduccionesPorCultivo: async (req, res) => {
		try {
			...
			const producciones = await Produccion.find({ cultivo_id }).populate(
				"cultivo_id",
				"nombre"
			);
			let totalCantidad = 0;
	...
	getProduccionesTotal: async (req, res) => {
		try {
			const totalProducciones =
				await Produccion.countDocuments().populate(
					"cultivo_id",
					"nombre"
				);
	...
	getProduccionesActivas: async (req, res) => {
		try {
			const producciones = await Produccion.find({ estado: 1 }).populate(
				"cultivo_id",
				"nombre"
			);
			res.json({ producciones });
	...
	getProduccionesInactivas: async (req, res) => {
		try {
			const producciones = await Produccion.find({ estado: 0 }).populate(
				"cultivo_id",
				"nombre"
			);
			res.json({ producciones });
	...
	putProduccion: async (req, res) => {
		try {
			const { id } = req.params;
			const { ...info } = req.body;
			const produccion = await Produccion.findByIdAndUpdate(id, info, {
				new: true,
			});
			res.json({ produccion });
		} catch (error) {
