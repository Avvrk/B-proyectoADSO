...
const httpParcelas = {
	getParcelas: async (req, res) => {
		try {
			const parcelas = await Parcela.find().populate(
				"id_fincas",
				"nombre rut"
			);
			res.json({ parcelas });
	...
	getParcelaActivos: async (req, res) => {
		try {
			const parcelas = await Parcela.find({ estado: 1 }).populate(
				"id_fincas",
				"nombre rut"
			);
			res.json({ parcelas });
	...
	getParcelaInactivos: async (req, res) => {
		try {
			const parcelas = await Parcela.find({ estado: 0 }).populate(
				"id_fincas",
				"nombre rut"
			);
			res.json({ parcelas });
	...
	postParcela: async (req, res) => {
		try {
			const {
				...
				asistenteTecnico,
				id_fincas,
			} = req.body; // Obtiene los datos de la parcela del cuerpo de la solicitud
	...
	putParcela: async (req, res) => {
		try {
			const { id } = req.params;
			const { ...info } = req.body;
			const parcela = await Parcela.findByIdAndUpdate(id, info, {
				new: true,
			});
			res.json({ parcela });
		} catch (error) {
