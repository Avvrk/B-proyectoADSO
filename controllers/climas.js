...
const httpClimas = {
	...
	getClimasPorClima: async (req, res) => {
		try {
			...
			const climas = await Clima.find({ tipoClima: clima })
				.populate("empleado_id", "nombre correo")
				.populate("finca_id", "nombre rut");
			res.json({ climas });
		} catch (error) {
	...
	getClimasPromedio: async (req, res) => {
		try {
			const climas = await Clima.find();
			const temperaturaMaxima = climas.map(
				(obj) => obj.temperaturaMaxima
			);
			const temperaturaMinima = climas.map(
				(obj) => obj.temperaturaMinima
			);
			const promedioMaxima =
				temperaturaMaxima.reduce((a, c) => a + c) /
				temperaturaMaxima.length;
			const promedioMinima =
				temperaturaMinima.reduce((a, c) => a + c) /
				temperaturaMinima.length;
	...
	putClimas: async (req, res) => {
		try {
			const { id } = req.params;
			const { ...info } = req.body;
			const climas = await Clima.findByIdAndUpdate(id, info, {
				new: true,
			});
			res.json({ climas });
		} catch (error) {
