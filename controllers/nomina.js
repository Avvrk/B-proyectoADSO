...
const httpNominas = {
	getNomina: async (req, res) => {
		try {
			const nomina = await Nomina.find().populate(
				"id_empleado",
				"nombre documento"
			);
			res.json({ nomina });
	...
	getNominaEmpleados: async (req, res) => {
		try {
			const { id } = req.params;
			const nomina = await Nomina.find({ id_empleado: id }).populate(
				"id_empleado",
				"nombre documento"
			);
			res.json({ nomina });
	...
	getNominaTotal: async (req, res) => {
		try {
			...
			const total = NominaTotal.reduce((acc, item) => {
	...
	putNomina: async (req, res) => {
		try {
			const { id } = req.params;
			const { ...info } = req.body;
			const nomina = await Nomina.findByIdAndUpdate(id, info, {
				new: true,
			});
			res.json({ nomina });
		} catch (error) {
