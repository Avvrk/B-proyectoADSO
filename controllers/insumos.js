...
const httpInsumos = {
    ...
    postInsumos: async (req, res) => {
        try {
            const { proveedores_id, nombre, relacionNPK, cantidad, unidad, responsable, observaciones, total } = req.body;
    ...
    putInsumos: async (req, res) => {
        try {
            const { id } = req.params;
            const { ...info } = req.body;
            const insumos = await Insumo.findByIdAndUpdate(id, info, { new: true });
            res.json({ insumos });
        } catch (error) {
            res.json({ error });
        }
