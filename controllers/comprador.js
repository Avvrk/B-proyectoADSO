...
const httpCompradores = {
	getCompradores: async (req, res) => {
		try {
			const compradores = await Comprador.find().populate({
				path: "_id_produccion",
				select: "cultivo_id",
				populate: { path: "cultivo_id", select: "nombre tipo" },
			});
	...
	getCompradoresCompras: async (req, res) => {
		try {
			...
			const compradores = await Comprador.find({ documento }).populate({
				path: "_id_produccion",
				select: "cultivo_id",
				populate: { path: "cultivo_id", select: "nombre tipo" },
			});
	...
	getCompradoresActivos: async (req, res) => {
		try {
			const compradores = await Comprador.find({ estado: 1 }).populate({
				path: "_id_produccion",
				select: "cultivo_id",
				populate: { path: "cultivo_id", select: "nombre tipo" },
			});
	...
	getCompradoresInactivos: async (req, res) => {
		try {
			const compradores = await Comprador.find({ estado: 0 }).populate({
				path: "_id_produccion",
				select: "cultivo_id",
				populate: { path: "cultivo_id", select: "nombre tipo" },
			});
	...
	putCompradores: async (req, res) => {
		try {
			const { id } = req.params;
			const { ...info } = req.body;
			const compradores = await Comprador.findByIdAndUpdate(id, info, {
				new: true,
			});
			res.json({ compradores });
		} catch (error) {
