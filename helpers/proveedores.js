...
const helpersproveedores = {
	validarProveedorID: async (proveedor_id) => {
		if (proveedor_id !== undefined) {
			if (!isMongoId(proveedor_id)) {
				throw new Error(
					"El campo proveedor_id debe ser un MongoID válido."
				);
			}
			try {
	...
	validarNombre: async (nombre = "") => {
		...
		if (nombreExistente) {
			throw new Error(
				"El nombre " + { nombre } + "ya se encuentra registrado."
			);
		}
	},
	...
	validarDireccion: (direccion) => {
		if (
			...
			throw new Error(
				"El campo dirección es obligatorio y debe ser una cadena de texto no vacía."
			);
		}
	...
	validarEstado: (estado) => {
		if (estado != undefined) {
			if (![0, 1].includes(Number(estado))) {
				throw new Error(
					"El campo estado debe ser 0 (inactivo) o 1 (activo)."
				);
			}
		}
	},
