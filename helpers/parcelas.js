...
const helpersParcela = {
	...
	validarUbicacionGeografica: (ubicacionGeografica) => {
		if (ubicacionGeografica != undefined) {
			if (
				typeof ubicacionGeografica !== "string" ||
	...
	validarAsistenteTecnico: (asistenteTecnico) => {
		if (asistenteTecnico != undefined) {
			if (
				typeof asistenteTecnico !== "string" ||
				asistenteTecnico.trim() === ""
			) {
				throw new Error(
					"El campo asistente técnico no debe estar vacío."
				);
			} else {
				return true;
			}
	...
	validarIdFincas: async (id_fincas) => {
		if (id_fincas != undefined) {
			if (!isMongoId(id_fincas)) {
				throw new Error(
					"El campo id_fincas debe ser un mongoId válido."
				);
			}
			try {
				...
				throw new Error(
					"Error al buscar la finca en la base de datos: " +
						error.message
				);
			}
		} else {
