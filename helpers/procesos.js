	...
	validarEmpleadoID: async (empleado_id) => {
		if (empleado_id !== undefined) {
			if (!isMongoId(empleado_id)) {
				throw new Error(
					"El campo empleado_id debe ser un MongoID válido."
				);
			}
			try {
	...
	validarFechaInicio: (fecha_inicio) => {
		if (fecha_inicio !== undefined) {
			if (!dateValido(fecha_inicio)) {
				throw new Error(
					"El campo fecha_inicio debe ser una fecha válida."
				);
			} else {
				return true;
			}
	...
	validarFechaFinal: (fecha_final) => {
		if (fecha_final !== undefined) {
			if (!dateValido(fecha_final)) {
				throw new Error(
					"El campo fecha_final debe ser una fecha válida."
				);
			} else {
				return true;
			}
	...
	validarProcesoID: async (id) => {
		if (id !== undefined) {
			...
			try {
				...
				throw new Error(
					"Error al buscar el proceso en la base de datos: " +
						error.message
				);
			}
		} else {
