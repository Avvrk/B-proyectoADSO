...
const helpersProducciones = {
	validarCultivoID: async (cultivo_id) => {
		if (cultivo_id !== undefined) {
			if (!isMongoId(cultivo_id)) {
				throw new Error(
					"El campo cultivo_id debe ser un MongoID válido."
				);
			}
			try {
				...
				throw new Error(
					"Error al buscar el cultivo en la base de datos: " +
						error.message
				);
			}
		} else {
	...
	validarCantidad: (cantidad) => {
		if (cantidad !== undefined) {
			if (typeof cantidad !== "number" || cantidad <= 0) {
				throw new Error(
					"El campo cantidad debe ser un número positivo."
				);
			} else {
				return true;
			}
	...
	validarCantidadTrabajadores: (cantidadTrabajadores) => {
		if (cantidadTrabajadores !== undefined) {
			if (
				typeof cantidadTrabajadores !== "number" ||
				cantidadTrabajadores < 0
			) {
				throw new Error(
					"El campo cantidadTrabajadores debe ser un número no negativo."
				);
			} else {
				return true;
			}
	...
	validarObservaciones: (observaciones) => {
		if (observaciones !== undefined) {
			if (typeof observaciones !== "string") {
				throw new Error(
					"El campo observaciones debe ser una cadena de texto."
				);
			} else {
				return true;
			}
	...
	validarProduccionID: async (id) => {
		if (id !== undefined) {
			if (!isMongoId(id)) {
				throw new Error(
					"El ID de producción debe ser un MongoID válido."
				);
			}
			try {
				...
				throw new Error(
					"Error al buscar la producción en la base de datos: " +
						error.message
				);
			}
		} else {
