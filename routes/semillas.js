...
router.post(
	...
	[
		check(
			...
			.notEmpty()
			.isMongoId(),
		check("numFactura", "El número de factura es requerido.")
			.optional()
			.notEmpty(),
		// check('fechaCompra').custom(helpersSemillas.validarFecha),
		// check('fechaVencimiento').custom(helpersSemillas.validarFecha),
		check("especieVariedad", "La especie/variedad es requerida.")
			.optional()
			.notEmpty(),
		check("numeroLote", "El número de lote es requerido.")
			.optional()
			.notEmpty(),
		check("origen", "El origen de la semilla es requerido.")
			.optional()
			.notEmpty(),
		check("poderGerminativo", "El poder germinativo es requerido.")
			.optional()
			.notEmpty(),
		check("observaciones", "Las observaciones son requeridas.")
			.optional()
			.notEmpty(),
		check("unidad", "La unidad es requerida.").optional().notEmpty(),
		check("total", "El total es requerido y debe ser un número válido.")
			.optional()
			.isNumeric(),
...
router.put(
	...
	[
		check(
			...
			.notEmpty()
			.isMongoId(),
		check("proveedor_id", "El ID del proveedor debe ser un mongoId válido.")
			.optional()
			.isMongoId()
			.custom(helpersSemillas.validarProveedorId),
		check(
			"numFactura",
			"El número de factura debe ser un valor válido."
		).optional(),
		// check('fechaCompra') .custom(helpersSemillas.validarFecha),
		// check('fechaVencimiento').custom(helpersSemillas.validarFecha),
		check(
			"especieVariedad",
			"La especie/variedad debe ser un valor válido."
		).optional(),
		check(
			"numeroLote",
			"El número de lote debe ser un valor válido."
		).optional(),
		check(
			"origen",
			"El origen de la semilla debe ser un valor válido."
		).optional(),
		check(
			"poderGerminativo",
			"El poder germinativo debe ser un valor válido."
		).optional(),
		check(
			"observaciones",
			"Las observaciones deben ser un valor válido."
		).optional(),
		check("unidad", "La unidad debe ser un valor válido.").optional(),
		check("total", "El total debe ser un número válido.")
			.optional()
			.isNumeric(),
		validarCampos,
		validarJWT,
