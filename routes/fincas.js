...
router.get("/", [validarJWT], httpFinca.getFincas);
...
router.get("/activos", [validarJWT], httpFinca.getFincasActivos);

router.get("/inactivos", [validarJWT], httpFinca.getFincasInactivos);
...
router.post(
	...
	[
		...
		check(
			"ubicacionGeografica",
			"La ubcacion geografica no puede estar vacia"
		).notEmpty(),
		check(
			"departamento",
		...
		check(
			"documentos",
			"El campo documentos no puede estar vacio"
		).notEmpty(),
