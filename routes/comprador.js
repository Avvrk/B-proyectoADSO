...
router.get("/", [validarJWT], httpComprador.getCompradores);
...
router.get("/activos", [validarJWT], httpComprador.getCompradoresActivos);

router.get("/inactivos", [validarJWT], httpComprador.getCompradoresInactivos);
...
router.get(
	...
	[
		...
		check(["fechaInicio", "fechaFin"]).custom(
			helpersComprador.validarFechas
		),
		validarCampos,
		validarJWT,
	],
...
router.post(
	...
	[
		check(
			"_id_produccion",
			"El id produccion no puede estar vacio"
		).notEmpty(),
		check(
			"_id_produccion",
			"Ingrese un mongo id de produccion valido"
		).isMongoId(),
		...
		check(
			"telefono",
			"El telefono debe tener minimo 10 caracteres"
		).isLength({ min: 10 }),
		...
		check(
			...
			"El numero guia transporte no puede estar vacio"
		).notEmpty(),
