...
router.get("/", [validarCampos, validarJWT], httpProveedores.getProveedores);
...
router.post(
	"/",
	[
		check("nombre", "El nombre es requerido y no puede estar vacío.")
			.notEmpty()
			.custom(helpersProveedores.validarNombre),
		check("direccion")
			.optional()
			.custom(helpersProveedores.validarDireccion),
		check("telefono").optional().custom(helpersProveedores.validarTelefono),
		check("email", "El email debe ser válido.")
			.isEmail()
			.custom(helpersProveedores.validarEmail),
		validarCampos,
		validarJWT,
	],
