...
router.get("/", [validarJWT], httpControlPlagas.getPlagas);
...
router.get(
	...
	[
		...
		check(["fechaInicio", "fechaFin"]).custom(
			herlpersControlPlagas.validarFechas
		),
		validarCampos,
		validarJWT,
	],
...
router.get("/tipo/:tipo", [validarJWT], httpControlPlagas.getPlagasTipo);
...
router.post(
	...
	[
		...
		check(
			"id_cultivo",
			"Ingrese un mongo id valido en el id cultivo"
		).isMongoId(),
		...
		check(
			"observaciones",
			"La observaciones no puede estar vacia"
		).notEmpty(),
		validarCampos,
		validarJWT,
	],
