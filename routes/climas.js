...
router.get("/", [validarJWT], httpClimas.getClimas);
...
router.get("/temperatura", [validarJWT], httpClimas.getClimasPromedio);
...
router.post(
	...
	[
		...
		check("promedio", "El tipo clima no puede estar vacio").notEmpty(),
		check("horaInicio", "La hora inicio no puede estar vacia").notEmpty(),
		check("horaInicio", "Ingrese una hora valida").matches(
			/^([01]\d|2[0-3]):([0-5]\d)$/
		),
		check("horaFinal", "La hora fin no puede estar vacia").notEmpty(),
		check("horaFinal", "Ingrese una hora valida").matches(
			/^([01]\d|2[0-3]):([0-5]\d)$/
		),
		check(
			"temperaturaMaxima",
			"La temperatura maxima no puede estar vacio"
		).notEmpty(),
		check(
			"temperaturaMaxima",
			"La temperatura maxina solo puede tener numeros"
		).isNumeric(),
		check(
			"temperaturaMinima",
			"La temperatura minia no puede estar vacia"
		).notEmpty(),
		check(
			"temperaturaMinima",
			"La temperatura minima solo puede tener numeros"
		).isNumeric(),
		...
		check(
			"pluviometria",
			"La pluviometria solo puede tener numeros"
		).isDecimal(),
		validarCampos,
		validarJWT,
	],
